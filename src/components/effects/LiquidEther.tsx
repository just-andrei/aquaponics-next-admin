...
/*
 * Upstream React Bits simulation internals use flexible pass/uniform objects.
 * Keep these exceptions scoped to this vendored WebGL component.
 */
...
function normalizedPalette(stops: string[]): string[] {
...
function paletteData(stops: string[]): Uint8Array {
  const palette = normalizedPalette(stops);
...
function makePaletteTexture(stops: string[]): THREE.DataTexture {
  const palette = normalizedPalette(stops);
  const texture = new THREE.DataTexture(paletteData(palette), palette.length, 1, THREE.RGBAFormat);
...
function updatePaletteTexture(texture: THREE.DataTexture, stops: string[]) {
  const palette = normalizedPalette(stops);
  texture.image.data = paletteData(palette);
  texture.image.width = palette.length;
...
export default function LiquidEther({
  ...
  const latestColorsRef = useRef(colors);
  latestColorsRef.current = colors;

  useEffect(() => {
    if (!mountRef.current) return;

    const paletteTex = makePaletteTexture(latestColorsRef.current);
...
`;
    ...
    return () => {
      ...
      paletteTex.dispose();
      if (paletteTextureRef.current === paletteTex) paletteTextureRef.current = null;
  ...
  useEffect(() => {
    if (paletteTextureRef.current) {
      updatePaletteTexture(paletteTextureRef.current, colors);
    }
  }, [colors]);
